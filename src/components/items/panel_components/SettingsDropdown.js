...
export default function SettingsDropdown({
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-gray-700 rounded-xl shadow-lg border border-gray-600 z-10">
          <div className="p-5 space-y-5">
            <label className="flex items-center justify-between cursor-pointer">
              <span className="text-base font-medium text-gray-200">☑️ 다중 선택 모드</span>
              <input
                type="checkbox"
                ...
                className="sr-only"
              />
              <div className={`w-12 h-6 rounded-full transition-colors ${isMultiSelect ? 'bg-blue-600' : 'bg-gray-500'}`}>
                <div className={`w-5 h-5 m-0.5 rounded-full bg-white transition-transform ${isMultiSelect ? 'translate-x-6' : ''}`} />
              </div>
            </label>
            {filteredItemsCount > 50 && (
              <label className="flex items-center justify-between cursor-pointer">
                <span className="text-base font-medium text-gray-200">⚡️ 가속 (가상화)</span>
                <input
                  type="checkbox"
                  ...
                  className="sr-only"
                />
                <div className={`w-12 h-6 rounded-full transition-colors ${isVirtualized ? 'bg-green-600' : 'bg-gray-500'}`}>
