...
const GridView = ({ items, handleSelect, categoryConfig, usageStats }) => (
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
    {items.map((node) => (
      <div key={node.id} className="flex flex-col h-full bg-white border border-gray-200 rounded-lg p-4 hover:border-blue-300 hover:shadow-md transition-all cursor-pointer" onClick={() => handleSelect(node.id)}>
...
const CompactView = ({ items, handleSelect, categoryConfig, currentChecklistItems }) => (
  <div className="p-4">
    {items.map((node) => (
      <div key={node.id} className="flex text-gray-900 items-center gap-3 py-2.5 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors border-b border-gray-100 last:border-b-0" onClick={() => handleSelect(node.id)}>
  ...
  if (filteredItems.length === 0) {
    return (
      <div className="text-center py-12 text-gray-500">
        ...
        <p className="text-lg font-medium mb-2">아직 항목이 없거나 검색 결과가 없습니다.</p>
