...
import { categoryConfig, colorThemes } from './items/constants';
...
const TreeItem = memo(function TreeItem({
  ...
  // 디버깅을 위한 로그 (개발 모드에서만) - 간소화
  if (process.env.NODE_ENV === 'development' && level <= 1) {
    console.log(`[TreeItem] ${node.id} (level:${level}) - expanded: ${currentIsExpanded}, hasChildren: ${hasChildren}`);
  }
  ...
  const handleToggleClick = (e) => {
    e.stopPropagation();
    console.log('Toggle click for:', node.id, 'hasChildren:', hasChildren, 'onToggleExpand:', !!onToggleExpand);
    if (hasChildren && onToggleExpand) {
      onToggleExpand(node.id);
      console.log('Called onToggleExpand for:', node.id);
    }
  };
